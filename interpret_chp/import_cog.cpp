...
segment import_segment(chp::graph &dst, const parse_cog::control &syntax, int default_id, tokenizer *tokens, bool auto_define) {
	...
	if (syntax.guard.valid) {
		segment sub = import_segment(dst, syntax.guard, syntax.kind, default_id, tokens, auto_define);
...
segment import_segment(chp::graph &dst, const parse::syntax *syntax, int default_id, tokenizer *tokens, bool auto_define) {
