...
pair<parse_astg::node, parse_astg::node> export_astg(parse_astg::graph &astg, const chp::graph &g, chp::iterator pos, map<chp::iterator, pair<parse_astg::node, parse_astg::node> > &nodes, string tlabel, string plabel)
{
	...
	if (loc == nodes.end()) {
		if (pos.type == chp::transition::type) {
			...
			parse_astg::expression guard = parse_cog::export_expression(g.transitions[pos.index].guard, g);
			parse_astg::composition action = parse_cog::export_composition(g.transitions[pos.index].action, g);
...
parse_astg::graph export_astg(const chp::graph &g) {
	...
	for (int i = 0; i < (int)g.vars.size(); i++)
		result.internal.push_back(parse_cog::export_expression(arithmetic::Expression::varOf(i), g));
	...
	for (int i = 0; i < (int)g.reset.size(); i++)
	{
		...
		result.marking.back().first = parse_cog::export_composition(arithmetic::Parallel(g.reset[i].encodings), g);
